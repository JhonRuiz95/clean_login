      ...
      onTap: () async {
        if (state.stateSolicitude == "") {
          Provider.of<FriendProvider>(context, listen: false)
              .createStateSolicitude(
                  getIt<MyUserData>().idUser, widget.friendId);
        }
        if (state.stateSolicitude == "pendiente1") {
          Provider.of<FriendProvider>(context, listen: false)
              .aceptarStateSolicitude(
                  [getIt<MyUserData>().idUser, widget.friendId]);
        }
        ...
        if (state.stateSolicitude == "pendiente") {
          _showMyDialogdelete(
              context, [getIt<MyUserData>().idUser, widget.friendId]);
        }
      },
